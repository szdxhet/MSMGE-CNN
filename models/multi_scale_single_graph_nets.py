...
class GraphEmbedding(nn.Module):
    ...
    def forward(self, x):#X(8,22,1126)
        ...
        ee = self.edge_weight.to(x.device)#231

        edge_weight[xx, yy] = ee#(22,22)
        edge_weight = edge_weight + edge_weight.T + th.eye(self.n_nodes, dtype=edge_weight.dtype, device=x.device)
        edge_weight = normalize_adj(edge_weight, mode='row')
        x_out = [x]
...
class EEGGENET(BaseModel):
    def __init__(self,
        ...
        self.ge = nn.Sequential(
            ...
            GraphEmbedding(self.in_chans, self.input_time_length, adj=self.Adj1, k=self.k, adj_learn=self.Adj_learn),
        )
    ...
    def forward_init(self, x):
        with th.no_grad():
            ...
            x1 = x1.view((self.k + 1), batch_size, -1, x1.size(-2), x1.size(-1))  # （3，1，8，22，1158）
            ...
            x2 = x2.view((self.k + 1), batch_size, -1, x2.size(-2), x2.size(-1))  # （3，1，8，22，1126）
            ...
            x3 = x3.view((self.k + 1), batch_size, -1, x3.size(-2), x3.size(-1))  # （3，1，8，22，1062）
    ...
    def forward(self, x):
        ...
        x1 = self.ge(x1)#（2，256 ，22，1158）
        x1 = x1.view((self.k + 1), batch_size, -1, x1.size(-2), x1.size(-1))#（2，32，8，22，1158）
        x1 = x1.permute(1, 0, 2, 3, 4).contiguous().view(batch_size, -1, x1.size(-2), x1.size(-1))#（32，16，22，1158）
        x1 = self.spatial_conv(x1)#（32，32，1，144）
        x1 = self.separable_conv(x1)#（32，16，1，18）

        x2 = self.temporal_conv2(x)  # (32,8,22,1126)
        x2 = self.ge(x2)  # （2，256 ，22，1126）
        x2 = x2.view((self.k + 1), batch_size, -1, x2.size(-2), x2.size(-1))  # （2，32，8，22，1126）
        x2 = x2.permute(1, 0, 2, 3, 4).contiguous().view(batch_size, -1, x2.size(-2), x2.size(-1))  # （32，16，22，1126）
        x2 = self.spatial_conv(x2)  # （32，32，1，140）
        x2 = self.separable_conv(x2)  # （32，16，1，17）

        x3 = self.temporal_conv3(x)  # (32,8,22,1062)
        x3 = self.ge(x3)  # （2，256 ，22，1062）
        x3 = x3.view((self.k + 1), batch_size, -1, x3.size(-2), x3.size(-1))  # （2，32，8，22，1062）
        x3 = x3.permute(1, 0, 2, 3, 4).contiguous().view(batch_size, -1, x3.size(-2), x3.size(-1))  # （32，16，22，1062）
        x3 = self.spatial_conv(x3)  # （32，32，1，132）
