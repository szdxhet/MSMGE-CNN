...
class EEGGENET(BaseModel):
    def __init__(self,
        ...
        self.ge = nn.Sequential(
            ...
            GraphEmbedding(self.in_chans, self.input_time_length, adj=self.Adj2, k=self.k, adj_learn=self.Adj_learn),
        )
        ...
        out = np_to_var(np.ones((1, self.in_chans, self.input_time_length, 1), dtype=np.float32))#(1,22,1125,1)
        out = self.forward_init(out)#(1,16,1,34)
        n_out_virtual_chans = out.cpu().data.numpy().shape[2]#n_out_virtual_chans:1

        if self.final_conv_length == 'auto':
            n_out_time = out.cpu().data.numpy().shape[3]#n_out_time:34
    ...
    def forward_init(self, x):
        with th.no_grad():
            batch_size = x.size(0)  # (1,22,1125,1)

            x = self.temporal_conv(x)  #  (1,8,22,1126)

            x = self.ge(x)  # （2，256 ，22，1126）
            x = x.view((self.k + 1), batch_size, -1, x.size(-2), x.size(-1))  # （2，32，8，22，1126）
